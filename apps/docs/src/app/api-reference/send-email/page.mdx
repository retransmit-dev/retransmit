...
[Send Batch](/api-reference/send-batch).
...
| `marketing` | boolean | No | Marks this as a marketing email: `{{{unsubscribe_url}}}` in the body becomes a hosted unsubscribe link, one-click `List-Unsubscribe` headers are added, and unsubscribed recipients are skipped. See [Marketing Email](/marketing-email). Defaults to `false`. |
