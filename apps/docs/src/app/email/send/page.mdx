...
| `tags` | array | No | Up to 10 `{ "name", "value" }` pairs for filtering sends in the dashboard, for example `{ "name": "campaign", "value": "outreach-1" }`. Names and values allow letters, digits, `_` and `-`, up to 256 characters. Names must be unique within one email. Tags are never sent to the recipient. |
